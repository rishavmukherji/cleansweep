...
class DiskScanner: ObservableObject {
    ...
    var totalReclaimable: Int64 { totalNodeModulesSize + totalCachesSize + totalAppDataSize }
    ...
    func scanAll() {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [self] in
            ...
            _scanDisk()
            setProgress("Scanning node_modules (this may take a moment)...")
            _scanNodeModules()
            setProgress("Scanning caches...")
            _scanCaches()
            setProgress("Scanning app data...")
            _scanAppData()
